...
import { apartmentData, featureGroupsData, featuresData } from "@/lib/site-data";
import type { ApartmentInfo, FeatureIconKey } from "@/lib/site-data";
...
function ApartmentPhotoPanel({ apartment }: { apartment: ApartmentInfo }) {
  ...
  return (
    <article className="overflow-hidden rounded-3xl border border-white/28 bg-white/12 shadow-[0_32px_80px_-44px_rgba(2,12,24,0.95)] backdrop-blur-md">
        ...
        {images.length > 1 ? (
          <>
            <button
              ...
              aria-label={`Voriges Bild ${apartment.title}`}
            >
              <ChevronLeft size={15} />
            </button>
            <button
              ...
              aria-label={`Naechstes Bild ${apartment.title}`}
            >
        ...
        <div className="flex items-center gap-1.5">
          {images.map((_, i) => (
            <button
              ...
              aria-label={`${apartment.title} Bild ${i + 1}`}
              className={`h-1.5 rounded-full transition-all duration-300 ${
                i === activeIndex ? "w-5 bg-slate-200/90" : "w-1.5 bg-slate-500/50 hover:bg-slate-400/60"
              }`}
            />
          ))}
        </div>
        <span className="text-[0.7rem] tabular-nums text-slate-300/70">
  ...
  return (
    <SectionShell id="ausstattung" className="section-band py-20 sm:py-28">
      ...
      <div className="mt-12 grid grid-cols-1 gap-8 lg:grid-cols-[1fr_400px] lg:gap-12 xl:grid-cols-[1fr_440px]">
        ...
        <MotionReveal delay={0.12} className="flex flex-col gap-5 lg:self-start">
          <ApartmentPhotoPanel apartment={apartmentData.b4} />
          <ApartmentPhotoPanel apartment={apartmentData.b14} />
        </MotionReveal>
      </div>
    </SectionShell>
  );
